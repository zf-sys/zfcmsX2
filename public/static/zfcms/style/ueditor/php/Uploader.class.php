<?php
// echo dirname(__FILE__); // /public/static/zfcms/style/ueditor_aliyunoss/php
// require '../../../../../../vendor/aliyuncs/oss-sdk-php/autoload.php';
// use OSS\OssClient as AliOssClient;
if(is_dir('../../../../../../vendor/aliyuncs/oss-sdk-php')){
...
class Uploader
{
    ...
    private function upFile()
    {
        ...
        //oss
        $upload_type = '';
        $web_data = require_once '../../../../../../config/web.php';
        $upload_type = $web_data['upload_type'];
        if($upload_type=='ali'){
            if(!is_dir('../../../../../../vendor/aliyuncs/oss-sdk-php')){
                $this->stateInfo = '未安装阿里云oss扩展';die;
            }
            $oss_data = require_once '../../../../../../config/oss.php';
            //oss设置
            $ossconfig = [
                'KeyId'      => $oss_data['ali_ak'],  //您的Access Key ID
                'KeySecret'  => $oss_data['ali_sk'],  //您的Access Key Secret
                'Bucket'     => $oss_data['ali_bucket'],  //Bucket名称
                'Endpoint'   => $oss_data['ali_domain'],  //阿里云oss 外网地址endpoint
                'ali_domain_diy'=>$oss_data['ali_domain_diy']
            ];
            //获取文件后缀
            ...
            $today = date('Ymd', time());
            //得到文件名
            $file_name = 'alioss/'.md5($today).'/'.$this->fileName;
            //$ossconfig为获取OSS的配置信息
            //$ossconfig = $this->getOssApi();
            //实例化OSS
            $ossClient = new OSS\OssClient($ossconfig['KeyId'], $ossconfig['KeySecret'], $ossconfig['Endpoint']);
            try {
                //执行阿里云上传
                $result = $ossClient->uploadFile($ossconfig['Bucket'],'zfcms/ueditor/'. $file_name, $file["tmp_name"]);
                $this->fullName = $result['info']['url'];
                //替换
                if($ossconfig['ali_domain_diy']!=''){
                    $this->fullName = str_replace('http://'.$ossconfig['Bucket'].'.'.$ossconfig['Endpoint'],$ossconfig['ali_domain_diy'],$this->fullName);
                }
